...
handle_cast({insert, Id, Key, Hash, Options}, S) ->
    S2 = do_insert(Id, Key, Hash, Options, S),
    {noreply, S2};

handle_cast({delete, BKey}, S) ->
    IndexN = get_index_n(BKey, yz_misc:get_ring(transformed)),
    S2 = do_delete(IndexN, term_to_binary(BKey), S),
    {noreply, S2};
